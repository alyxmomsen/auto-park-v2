	...
	switch (action.type) {
		...
		case PURGE_PENDING_MESSAGES:
			return {
				...state,
				pendingMessages: [
					...state.pendingMessages.filter((elem) => !!!action.payload.Ids.find((el) => el === elem.Id)),
				],
			};
		default:
			return state;
...
const Tracker = () => {
	const url = {
		timeweb: ['http://85.234.106.173:3333/', 'http://85.234.106.173:3003/'],
		local: ['http://localhost:3333', 'http://localhost:3003'],
	};
	...
	const [trackerState, dispatchTrackerState] = useReducer<
		(state: TrackerState, action: TrackerAction) => TrackerState
	>(trackerReducer, {
		pendingMessages: [],
		postedMessages: [],
		socket: {
			...
			<div>
				...
				<div className={'messenger-root'}>
					<div className={'messenger-root__controller'}>
						<div>{trackerState.socket.connection.state ? 'connected' : 'no connected'}</div>
						<div>
							{trackerState.pendingMessages.map((message) => (
								<div>
									<div>{message.Id}</div>
					...
					<div className={'messenger-root__chat'}>
						{trackerState.postedMessages.reverse().map((message) => (
